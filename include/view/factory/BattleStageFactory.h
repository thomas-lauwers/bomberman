...
public:
    ...
    /** @brief Creates an AI bomber entity. @param x X coordinate. @param y Y coordinate. @param type The AI bomber
     * type. @return Unique pointer to the AI bomber entity. */
    std::unique_ptr<AIBomber> createAIBomber(float x, float y, BomberType type) override;
    /** @brief Creates a bomb entity. @param x X coordinate. @param y Y coordinate. @param blast_radius The radius of
     * the blast. @return Unique pointer to the bomb entity. */
    std::unique_ptr<Bomb> createBomb(float x, float y, int blast_radius) override;
    /** @brief Creates a destructible wall entity. @param x X coordinate. @param y Y coordinate. @return Unique pointer
     * to the destructible wall entity. */
    std::unique_ptr<DestructibleWall> createDestructibleWall(float x, float y) override;
    /** @brief Creates an explosion entity. @param x X coordinate. @param y Y coordinate. @param type The type of the
     * explosion. @return Unique pointer to the explosion entity. */
    std::unique_ptr<Explosion> createExplosion(float x, float y, ExplosionType type) override;
    /** @brief Creates a crumbling wall entity. @param x X coordinate. @param y Y coordinate. @return Unique pointer to
     * the crumbling wall entity. */
    std::unique_ptr<CrumblingWall> createCrumblingWall(float x, float y) override;
    /** @brief Creates a power-up entity. @param x X coordinate. @param y Y coordinate. @return Unique pointer to the
     * power-up entity. */
    std::unique_ptr<PowerUp> createPowerUp(float x, float y) override;
    /** @brief Creates a knocked-out bomber entity. @param x X coordinate. @param y Y coordinate. @param type The bomber
     * type. @return Unique pointer to the knocked-out bomber entity. */
