...
std::unique_ptr<AIBomber> BattleStageFactory::createAIBomber(const float x, const float y, BomberType type) {
    ...
    return createEntity(std::move(aiBomber), [this, type](AIBomber* e) { return renderer.createAIBomberView(e, type); });
}

std::unique_ptr<Bomb> BattleStageFactory::createBomb(float x, float y, int blast_radius) {
    return createEntity(std::make_unique<Bomb>(x, y, blast_radius), [this](Bomb* e) { return renderer.createBombView(e); });
}
...
std::unique_ptr<Explosion> BattleStageFactory::createExplosion(float x, float y, ExplosionType type) {
    return createEntity(std::make_unique<Explosion>(x, y, type), [this](Explosion* e) { return renderer.createExplosionView(e); });
}

std::unique_ptr<CrumblingWall> BattleStageFactory::createCrumblingWall(float x, float y) {
    return createEntity(std::make_unique<CrumblingWall>(x, y), [this](CrumblingWall* e) { return renderer.createCrumblingWallView(e); });
}

std::unique_ptr<PowerUp> BattleStageFactory::createPowerUp(float x, float y) {
    return createEntity(std::make_unique<PowerUp>(x, y), [this](PowerUp* e) { return renderer.createPowerUpView(e); });
}

std::unique_ptr<KnockedOutBomber> BattleStageFactory::createKnockedOutBomber(float x, float y, BomberType type) {
    return createEntity(std::make_unique<KnockedOutBomber>(x, y, type), [this, type](KnockedOutBomber* e) { return renderer.createKnockedOutBomberView(e, type); });
}
